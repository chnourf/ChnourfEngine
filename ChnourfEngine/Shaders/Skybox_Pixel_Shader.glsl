...
{  
    ...
    float spot = smoothstep(0.0, 200.0, phase(sundot, 0.9995)) * 15000.0;
   
    float eye_height = 1-30.0/6400.0;
    vec3 eye_position = vec3(0.0, eye_height, 0.0);
    float eye_depth = atmospheric_depth(eye_position, rd);
    float step_length = eye_depth/float(step_count);
    float eye_extinction = horizon_extinction(eye_position, rd, eye_height - 0.2);
    ...
    for (int i=0; i<step_count; i++)
    {
        ...
        float extinction = horizon_extinction( position, normalizedToLightDirection, eye_height-0.3);
    ...
    rayleigh_collected = (rayleigh_collected * eye_extinction * eye_depth * Kr * 1/22.4e-6)/float(step_count);
