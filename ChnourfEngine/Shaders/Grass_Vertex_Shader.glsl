...
{	
	...
	vec3 offset = vec3((in_quad_position.x - 0.5) * cos(direction), in_quad_position.y, (in_quad_position.x - 0.5) * sin(direction)) * (misc.z + 128)/255;
	...
	vs_out.texcoord = -in_quad_position + vec2(0.5);
}