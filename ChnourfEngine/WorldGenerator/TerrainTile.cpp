...
TerrainTile::TerrainTile(const vec2i& aGridIndex, unsigned int aTileSize, float aResolution) :
...
float TerrainTile::GetY(float aX, float aZ) const
{
	...
	unsigned          ix0 = glm::clamp(unsigned(fx), 0u, myTileSize - 1u);

	float  fz = v / myResolution;
	unsigned          iz0 = glm::clamp(unsigned(fz), 0u, myTileSize - 1u);

	unsigned ix1 = glm::min(ix0 + 1u, myTileSize - 1u);
	unsigned iz1 = glm::min(iz0 + 1u, myTileSize - 1u);
