...
{
	float  u = (aX - myGridIndex.x * myCellSize);
	float  v = (aZ - myGridIndex.y * myCellSize);
	if (u > 0 && u < (int)myCellSize - 1 && v > 0 && v < (int)myCellSize -1)
	{
		...
		float height0 = myElements[iz0 * myCellSize + ix0].myElevation; // (0,0)
		float height1 = myElements[iz1 * myCellSize + ix0].myElevation; // (0,1)
		float height2 = myElements[iz1 * myCellSize + ix1].myElevation; // (1,1)
		float height3 = myElements[iz0 * myCellSize + ix1].myElevation; // (1,0)
