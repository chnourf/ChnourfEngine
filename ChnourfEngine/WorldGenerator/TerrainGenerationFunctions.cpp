	...
	void ComputeErosion(std::vector<TerrainElement>& elevationMap, const unsigned int iterations, const TerrainGeneration::ErosionParams& params, const unsigned int& aCellSize, std::default_random_engine aRandomEngine)
{
	...
	const unsigned int MAX_PATH_LEN = aCellSize * 4;

#define DEPOSIT_AT(X, Z, W) \
	{ \
	float delta=ds*(W); \
	elevationMap [X + aCellSize * Z].myElevation  += delta; \
	}

#define DEPOSIT(H) \
	for (int z = zi - 1; z <= zi + 2; ++z)\
	{\
		if (z < 0 || z > aCellSize - 1)\
			continue;\
		float zo = z - zp;\
		float zo2 = zo*zo;\
		for (int x = xi - 1; x <= xi + 2; ++x)\
		{\
			if (x < 0 || x > aCellSize - 1)\
				continue;\
			float xo = x - xp;\
	...
	for (unsigned int iter = 0; iter < iterations; ++iter)
	{
		...
		float h = elevationMap[xi + aCellSize * zi].myElevation;
		float carriedSoil = 0, speed = 1, water = 1;

		float h00 = h;
		float h10 = elevationMap[xi + 1 + aCellSize * zi].myElevation;
		float h01 = elevationMap[xi + aCellSize * (zi + 1)].myElevation;
		float h11 = elevationMap[(xi + 1) + aCellSize * (zi + 1)].myElevation;
		...
		for (; numMoves < MAX_PATH_LEN; ++numMoves)
		{
			...
			// sample next height
			int nxi = glm::clamp((int)std::floor(nxp), 0, (int)aCellSize - 1);
			int nzi = glm::clamp((int)std::floor(nzp), 0, (int)aCellSize - 1);

			// the drop falls of the cell
			if (nxi == aCellSize - 1 || nzi == aCellSize - 1 || nxi == 0 || nzi == 0)
				break;

			float nxf = nxp - nxi;
			float nzf = nzp - nzi;

			float nh00 = elevationMap[nxi + aCellSize * nzi].myElevation;
			float nh10 = elevationMap[nxi + 1 + aCellSize * nzi].myElevation;
			float nh01 = elevationMap[nxi + aCellSize * (nzi + 1)].myElevation;
			float nh11 = elevationMap[nxi + 1 + aCellSize * (nzi + 1)].myElevation;
